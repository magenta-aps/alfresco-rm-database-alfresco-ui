...
function AlfrescoDownloadService(alfrescoNodeUtils, ALFRESCO_URI, sessionService) {
    ...
    function downloadFile(nodeRef, fileName,attachment) {

        var url = ALFRESCO_URI.webClientServiceProxy + "/api/node/content/" + alfrescoNodeUtils.processNodeRef(nodeRef).uri + "/" + fileName + '?a=' + attachment;
